...
def create_summary_file(commit=None):
    ...
    with open('./summary.json', 'w') as ff:
...
def negative_binomial_regression():
    ...
    # Exclude repos with 0 bugs
    df = df[(df['bugs'] > 0)]
    print(df)
    print(df.describe())
    df.to_clipboard()

    # This was originally used to generate the masks, after which we hardcoded it to eliminate randomness.
    ...
    # The masks to determine which datapoint gets put in the trainig or test set
