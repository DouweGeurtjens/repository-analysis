...
def _landau_density(x, mu, sigma):
    ...
    """
    fn = lambda t: np.exp(-t * ((x - mu) / sigma) -
                          (2 / np.pi) * t * np.log(t)) * np.sin(2 * t)
    return 1 / (mu * sigma) * integrate.quad(
        fn, 0, np.inf
    )[0]  # Note: may need to increase upper bound for sub intervals ('limit' keyword for quad)
...
print(hmp([0.002, 0.002, 0.007, 0.003]))
print(hmp([0.124, 0.398, 0.241, 0.229, 0.398]))


print(np.average([0.0099,0.0094,0.0106,0.0094,0.0105]))
print(np.average([1.43,1.501,1.407,1.257,1.341]))
print(np.average([-1.428,-0.837,-1.371,-1.143,-0.805]))