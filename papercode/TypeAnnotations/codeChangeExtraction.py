...
import config
from papercode.TypeAnnotations.codeChange import CodeChange, SingleDiffChange
from papercode.TypeErrors.TypeAnnotationCounter import compute_vars_without_types
from papercode.parsers import TypeCollector
...
def extract_from_snippet(string):
    try:
        ...
        non_variable_types = compute_vars_without_types(
            scopes, type_collector.names_with_type)
...
def extract_from_snippet_new_new(string_old, string_new):
    ...
    if not ("fatal: Path" in str(string_old)
            and "does not exist in" in str(string_old)):
        ast_old = cst.parse_module(string_old)
        wrapper_old = cst.metadata.MetadataWrapper(ast_old)
        positions_old = wrapper_old.resolve(PositionProvider)
                                ...
                                if hasattr(parameter.annotation, 'annotation'):
                                    if hasattr(parameter.annotation.annotation,
                                        ...
                                        if hasattr(
                                            ...
                                            if parameter.annotation.annotation.value.value.lower(
                                            ) in ['list', 'tuple', 'range']:
                                                brackets = "["
                                                for child_type in parameter.annotation.annotation.children:
                                                    if 'SubscriptElement' in type(
                                                            child_type
                                                    ).__name__:
                                            ...
                                            # print('[ARG] ', parameter.annotation.annotation.value.value, '->', pos)
                                            annotation_node = SingleDiffChange(
                                                'argument', 'old',
                                                parameter.name.value,
                                                parameter.annotation.
                                                annotation.value.value +
                                                brackets, pos.start.line)
                                            node_list_old.append(
                                                annotation_node)
                                            brackets = ""
                                        else:
                    ...
                    if hasattr(node, 'returns'):
                        if hasattr(node.returns, 'annotation'):
                            if hasattr(node.returns.annotation, 'value'):
                                if hasattr(node.returns.annotation.value,
                                           'value'):
                                    if node.returns.annotation.value.value.lower(
                                    ) in ['list', 'tuple', 'range']:
                                        brackets = "["
                                ...
                                # print('[RETURN] ', node.returns.annotation.value, '->', pos)
                                if hasattr(node.returns.annotation.value,
                                           'value') and 'Name' in type(
                                               node.returns.annotation.value
                                           ).__name__:
                                    annotation_node = SingleDiffChange(
                                        'return', 'new', node.name.value,
                                        node.returns.annotation.value.value +
                                        brackets, pos.start.line)
                                else:
                                    annotation_node = SingleDiffChange(
                                        'return', 'new', node.name.value,
                                ...
                                if hasattr(variable.annotation, 'annotation'):
                                    if hasattr(variable.annotation.annotation,
                                        ...
                                        if hasattr(
                                                ...
                                                value, 'value'):
                                            if variable.annotation.annotation.value.value.lower(
                                            ) in ['list', 'tuple', 'range']:
                                                brackets = "["
    ...
    if not ("fatal: Path" in str(string_new)
            and "does not exist in" in str(string_new)):
        ast_new = cst.parse_module(string_new)
        wrapper_new = cst.metadata.MetadataWrapper(ast_new)
        positions_new = wrapper_new.resolve(PositionProvider)
...
def extract_from_snippet_new_new_new(string_old, string_new, file_old,
                                     file_new):
    # if len(string_old) == 0:
    #   return {}, {}, {}
                ...
                if hasattr(node, 'annotation'):
                    if hasattr(node.annotation, 'annotation'):
                        if hasattr(node.annotation.annotation, 'value'):
                            if hasattr(node.annotation.annotation.value,
                                       'value'):
                                if node.annotation.annotation.value.value.lower(
                                ) in ['list', 'tuple', 'range']:
                                    brackets = collection_type_annotation_recursive(
                                        node.annotation.annotation.slice
                                    ).replace("][", ",")

                                # print('[ARG] ', parameter.annotation.annotation.value.value, '->', pos)
                                annotation_node = SingleDiffChange(
                                    'argument', 'new', file_old,
                                    node.name.value,
                                    node.annotation.annotation.value.value +
                                    brackets, pos.start.line)
                                node_list_old.append(annotation_node)
                                brackets = ""
                            else:
                    ...
                    if hasattr(node.returns, 'annotation'):
                        if hasattr(node.returns.annotation, 'value'):
                            ...
                            if hasattr(node.returns.annotation.value,
                                ...
                                annotation_node = SingleDiffChange(
                                    'return', 'new', file_old, node.name.value,
                                    node.returns.annotation.value.value +
                                    brackets, pos.start.line)
                            else:
                                annotation_node = SingleDiffChange(
                                    ...
                                    if hasattr(variable.annotation.annotation,
                                        ...
                                        if hasattr(
                                            ...
                                            if variable.annotation.annotation.value.value.lower(
                                                ...
                                                brackets = collection_type_annotation_recursive(
                                                    variable.annotation.
                                                    annotation.slice).replace(
                                                        "][", ",")

                                            # print('[VAR] ', variable.annotation.annotation.value.value, '->', pos)
                                            annotation_node = SingleDiffChange(
                                                'variable', 'new', file_old,
                                                variable.target.value,
                                                variable.annotation.annotation.
                                                value.value + brackets,
                                                pos.start.line)
                    ...
                    if hasattr(node.returns, 'annotation'):
                        if hasattr(node.returns.annotation, 'value'):
                            ...
                            if hasattr(node.returns.annotation.value,
                                ...
                                annotation_node = SingleDiffChange(
                                    'return', 'new', file_new, node.name.value,
                                    node.returns.annotation.value.value +
                                    brackets, pos.start.line)
                            else:
                                annotation_node = SingleDiffChange(
...
def extract_from_snippet_new_new_new_new(string_old, string_new, file_old,
                                         file_new):
                        ...
                        if hasattr(node.annotation.annotation, 'value'):
                            ...
                            if hasattr(node.annotation.annotation, 'attr'):
                                if hasattr(node.annotation.annotation.attr,
                                           'value'):
                                    attribute = "." + str(
                                        node.annotation.annotation.attr.value)
                            ...
                            if hasattr(node.annotation.annotation.value,
                                ...
                                annotation_node = SingleDiffChange(
                                    'argument', 'new', file_old,
                                    node.name.value,
                                    str(node.annotation.annotation.value.value)
                                    + str(brackets) + attribute,
                                    pos.start.line)
                                node_list_old.append(annotation_node)
                                brackets = ""
                        ...
                        if hasattr(node.annotation.annotation, 'value'):
                            ...
                            if hasattr(node.annotation.annotation.value,
                                ...
                                annotation_node = SingleDiffChange(
                                    ...
                                    node.name.value,
                                    str(node.annotation.annotation.value.value)
                                    + brackets + attribute, pos.start.line)
                                node_list_new.append(annotation_node)
                                brackets = ""
                            else:
                                # print('[ARG2] ', parameter.annotation.annotation.value, '->', pos)
    ...
    if size_old == (len(node_list_old) +
                    len(annotation_list_old)) and size_new == (
                        len(node_list_new) + len(annotation_list_new)):
        ...
        for child in slice:
            if hasattr(child, 'value'):
                if hasattr(child.value, 'slice'):
                    type_ann += collection_type_annotation_recursive(
                        child.value.slice)
                ...
                if hasattr(slice.value.value, 'value'):
                    if hasattr(slice.value.value.value, 'value'):
                        if hasattr(slice.value.value.value.value, 'value'):
                            return "[" + str(slice.value.value.value.value.
                                             value) + str(type_ann) + "]"
                        else:
                            return "[" + str(slice.value.value.value.value
                                             ) + str(type_ann) + "]"
                        ...
                        return "[" + str(
                            slice.value.value.value) + str(type_ann) + "]"
                else:
...
def search_key_value_in_snippet(file, list_of_strings) -> int:
    ...
    try:
        ...
        for line in file.splitlines():
            ...
            l = line
            if strr in line and "".join(list_of_strings[1]) in line.replace(
                    " ", "") and (':' in l or '->' in l):
                # If any string is found in line, then append that line along with line number in list
                return line_number, l.rstrip()
...
def search_line_number_param(file, dict):
    try:
        ...
        for line in file.splitlines():
            ...
            list_temp = []
            for key in key_list:
                regex_temp = (re.escape(key[-1]) + "\s*:\s*" +
                              re.escape(dict[key])).replace(
                                  "[\\]", "[(.*?)\\]")
                ...
                if len(key) > 2:
                    key_temp = key[-3]
                    if re.search("class\s*" + re.escape(key_temp) + "\s*:\s*",
                                 line):
                ...
                if re.search(
                        regex_temp, line.replace(" ", "")
                ) and key_temp in class_visited and key[-2] == method_temp:
    ...
    try:
        ...
        for line in file.splitlines():
            ...
            for key in key_list:
                ...
                if re.search(
                        regex_temp, line.replace(" ", "")
                ) and key_temp in class_visited and key[-1] == method_temp:
...
def TypeAnnotationExtractionLast(
        repo_path,
        repo_name,
        commit,
        patch,
    ...
    old_out = subprocess.Popen([
        "git", "--git-dir",
        str(repo_path + repo_name) + '/.git', 'show',
    ...
    try:
        ...
        for hunk in patch.hunks:
            ...
            for annotation_old in node_list_old:
                if annotation_old.line not in range(
                ...
                for annotation_new in node_list_new:
                    j += 1
                    if annotation_new.line not in range(
                    ...
                    if annotation_old.type == annotation_new.type and annotation_old.variable == annotation_new.variable:
                        ...
                        if hasattr(annotation_old.annotation, 'value'):
                            annotation_old.annotation = str(
                                annotation_old.annotation.value)

                        if hasattr(annotation_new.annotation, 'value'):
                            annotation_new.annotation = str(
                                annotation_new.annotation.value)
                        ...
                        temp = CodeChange(url + 'L' + str(annotation_old.line),
                                          str(commit_year), "", "", "0",
                                          str(annotation_old.type),
                                          '[CHANGED]', annotation_old.variable,
                        ...
                        statistics.number_type_annotations_per_repo[
                            repo_name] += 1
                        statistics.total_typeAnnotation_codeChanges += 1

                        if commit_year not in statistics.modify_existing_types:
                            statistics.modify_existing_types[str(
                                commit_year)] = 1
                        else:
                            statistics.modify_existing_types[str(
                                commit_year)] += 1

                        line_type_annotation_changed.append(
                            str(annotation_old.line))

                        if str(annotation_old.type) == 'argument':
                            if (annotation_old.annotation +
                                    ' -> ' + annotation_new.annotation).lower(
                                    ) not in statistics.typeChanged_dict_arg:
                                statistics.typeChanged_dict_arg[
                                    str(annotation_old.annotation + ' -> ' +
                                        annotation_new.annotation).lower()] = 1
                            else:
                                statistics.typeChanged_dict_arg[str(
                                    str(annotation_old.annotation + ' -> ' +
                                        annotation_new.annotation).lower()).
                                                                lower()] += 1
                            ...
                            if (annotation_old.annotation +
                                    ' -> ' + annotation_new.annotation).lower(
                                    ) not in statistics.typeChanged_dict_ret:
                                statistics.typeChanged_dict_ret[
                                    str(annotation_old.annotation + ' -> ' +
                                        annotation_new.annotation).lower()] = 1
                            else:
                                statistics.typeChanged_dict_ret[str(
                                    str(annotation_old.annotation + ' -> ' +
                                        annotation_new.annotation).lower()).
                                                                lower()] += 1
                            ...
                            if (annotation_old.annotation +
                                    ' -> ' + annotation_new.annotation).lower(
                                    ) not in statistics.typeChanged_dict_var:
                                statistics.typeChanged_dict_var[
                                    str(annotation_old.annotation + ' -> ' +
                                        annotation_new.annotation).lower()] = 1
                            else:
                                statistics.typeChanged_dict_var[str(
                                    str(annotation_old.annotation + ' -> ' +
                                        annotation_new.annotation).lower()).
                                                                lower()] += 1
                ...
                if flag_insert:
                    ...
                    temp = CodeChange(
                        url + 'L' + str(annotation_old.line), str(commit_year),
                        "", f"{commit_year}-{commit_month}-{commit_day}", "0",
                        str(annotation_old.type),
                        '[REMOVED]', annotation_old.variable,
                        str(patch.delta.old_file.path),
                        str(annotation_old.annotation),
                        str(annotation_old.line), ' ', ' ', ' ')
                    ...
                    line_type_annotation_removed.append(
                        str(annotation_old.line))

                    if annotation_old.annotation.lower(
                    ) not in statistics.typeRemoved_dict:
                        statistics.typeRemoved_dict[
                            annotation_old.annotation.lower()] = 1
                    else:
                        statistics.typeRemoved_dict[
                            annotation_old.annotation.lower()] += 1
            ...
            for remained in node_list_new:
                if remained.line in range(hunk.new_start,
                    ...
                    temp = CodeChange(
                        url + 'R' + str(remained.line), str(commit_year),
                        f"{commit_year}-{commit_month}-{commit_day}", "", "0",
                        str(remained.type), '[INSERTED]', remained.variable,
                        ' ', ' ', ' ', str(patch.delta.new_file.path),
                        str(remained.annotation), str(remained.line))
                    ...
                    if remained.annotation.lower(
                    ) not in statistics.typeAdded_dict:
                        statistics.typeAdded_dict[
                            remained.annotation.lower()] = 1
                    else:
                        statistics.typeAdded_dict[
                            remained.annotation.lower()] += 1
    ...
    if len(line_type_annotation_removed) > 0:
        typeannotation_line_removed[0] += len(
            set(line_type_annotation_removed))
        list_line_removed[0] += len(set(line_type_annotation_removed))

    if len(line_type_annotation_changed) > 0:
        typeannotation_line_inserted[0] += len(
            set(line_type_annotation_changed))
        typeannotation_line_removed[0] += len(
            set(line_type_annotation_changed))
        typeannotation_line_changed[0] += len(
            set(line_type_annotation_changed))
...
def TypeAnnotationExtractionLast_life(
        repo_path,
        repo_name,
        commit,
        patch,
    ...
    try:
        ...
        node_list_old, node_list_new = extract_from_snippet_new_new_new_new(
            str(old_stdout.decode('utf-8-sig')),
            str(new_stdout.decode('utf-8-sig')),
            str(patch.delta.old_file.path), str(patch.delta.new_file.path))
    except Exception as e:
        #print(str(e), repo_path, commit.hex)
        return
        ...
        for hunk in patch.hunks:
                ...
                if flag_insert:
                    ...
                    for elem in code_changes:
                        if elem.where == annotation_old.type and elem.variable == annotation_old.variable and elem.new_file == annotation_old.filename:
                            ...
                            elem.life_time = str(
                                days_between(elem.creation_date,
                                             elem.elimination_date))
            ...
            for remained in node_list_new:
                if remained.line in range(hunk.new_start,
                    ...
                    temp = CodeChange(
                        url + 'R' + str(remained.line), "", "",
                        str(commit_year),
                        f"{commit_year}-{commit_month}-{commit_day}", "", "-1",
                        "0", str(remained.type), '[INSERTED]',
                        '' if 'Name' in type(
                            remained.variable).__name__ else remained.variable,
                        ' ', ' ', ' ', str(patch.delta.new_file.path),
                        str(remained.annotation), str(remained.line))
...
def TypeAnnotationExtractionFirstCommit(
        repo_path,
        repo_name,
        commit,
        patch,
    ...
    try:
        ...
        for key in new_return_types:
            ...
            temp = CodeChange(url + str(new_line), commit_year,
                              str(patch.delta.old_file.path), '', '',
                              str(patch.delta.new_file.path), new_line,
                              str(new_return_types[key]))
        ...
        for key in new_param_types:
            ...
            for key in new_variable_types:
                ...
                if new_variable_types[key].lower(
                ) not in statistics.typeAdded_dict:
                    statistics.typeAdded_dict[
                        new_variable_types[key].lower()] = 1
                else:
                    statistics.typeAdded_dict[
                        new_variable_types[key].lower()] += 1
                statistics.total_added += 1
...
def type_annotation_in_last_version(repo_name, statistics):  # , lock):
    ...
    for filepath in pathlib.Path(config.ROOT_DIR + "/GitHub/" +
                                 repo_name).glob('**/*'):
        temp = []
        if str(filepath).endswith(".py"):
            try:
                param_types, return_types, variable_types = extract_from_file(
                    str(filepath))

                if (not param_types) and (not return_types) and (
                        not variable_types):
                    continue
                ...
                if repo_name not in statistics.typeLastProjectVersion_dict:
                    statistics.typeLastProjectVersion_dict[repo_name] = len(
                        temp)
                else:
                    statistics.typeLastProjectVersion_dict[repo_name] += len(
                        temp)
...
def retype_stub(repo_name, filename, commit):
    ...
    subprocess.run(f"git checkout {commit} --quiet".split(" "),
                   cwd=config.ROOT_DIR + "/GitHub/" + repo_name)
    ...
    for key in dict_files.keys():
        ...
        for i in llll:
            if len(i) > 0:
                index = i.rindex('/')
                lll = i[:index]
                os.system('/home/user/anaconda3/bin/retype -p ' + lll +
                          ' -t ' + config.ROOT_DIR + "/Test/" + ' ' + key)
    ...
    for key in dict_files.keys():
        ...
        for i in llll:
            if len(i) > 0:
                index = i.rindex('/')
                lll = i[:index]
                print('\n\n/home/user/anaconda3/bin/retype -p ' + lll +
                      ' -t ' + config.ROOT_DIR + "/GitHub/" + repo_name + ' ' +
                      key)
                os.system('/home/user/anaconda3/bin/retype -p ' + lll +
                          ' -t ' + config.ROOT_DIR + "/GitHub/mypy" +
                          repo_name + ' ' + key)
