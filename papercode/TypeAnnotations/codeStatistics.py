...
import numpy as np
import config
from papercode.TypeAnnotations.Utils import sort_dictionary, merge_dictionaries
...
class CodeStatistics:
    ...
    def statistics_computation(self):
        ...
        # [RQ6]: Which are the top 10 repository with the highest number of type annotations
        self.number_type_annotations_per_repo = sort_dictionary(
            self.number_type_annotations_per_repo)

        # [RQ7]: How many of all types are annotated in the last verison of the code?
        if self.total_typeAnnotation_codeChanges > 0:
            self.typeLastProjectVersion_average = sum(
                self.typeLastProjectVersion_percentage) / len(
                    self.typeLastProjectVersion_percentage)

        #   self.typeLastProjectVersion_percentage.append(
        #      round(self.typeLastProjectVersion_total / self.insert_types * 100, 2))

        self.typeLastProjectVersion_dict = sort_dictionary(
            self.typeLastProjectVersion_dict)

    # [RQ1]: Are type annotation inserted, removed and changed?
    def percentage_computation(self):
        self.total_typeAnnotation_codeChanges = sum(
            self.insert_types.values()) + sum(
                self.remove_types.values()) + sum(
                    self.modify_existing_types.values())

        self.percentage_repo_with_typeChanges = str(
            round(self.repo_with_types_changes / self.total_repositories * 100,
                  2)) + ' %'
        self.percentage_commits_with_typeChanges = str(
            round(self.commits_with_typeChanges / self.total_commits * 100,
                  2)) + ' %'
        self.percentage_insert_types = str(
            round(
                sum(self.insert_types.values()) /
                self.total_typeAnnotation_codeChanges * 100, 1)) + ' %'
        self.percentage_remove_types = str(
            round(
                sum(self.remove_types.values()) /
                self.total_typeAnnotation_codeChanges * 100, 1)) + ' %'
        self.percentage_modify_existing_types = str(
            round(
                sum(self.modify_existing_types.values()) /
                self.total_typeAnnotation_codeChanges * 100, 1)) + ' %'
    ...
    def what_types_added(self):
        ...
        for type in self.typeAdded_dict.keys():
            if any(x in type
                   for x in self.sequenceType_list) and '->' not in type:
        ...
        for type in self.typeAdded_dict.keys():
            if any(x in type
                   for x in self.binarySequenceType_list) and '->' not in type:
        ...
        for type in self.typeAdded_dict.keys():
            if any(x in type
                   for x in self.mappingType_list) and '->' not in type:
    ...
    # [RQ5]: Relation of properties of projects vs. properties of type changes
    # E.g., nb of stars/developers/overall commits vs. nb of added annotations
    def addRepo(self, name, n_commits, n_annotations, n_test_files,
                n_non_test_files, n_dev, funct_type_avg, fuct_no_type_avg):
        i = 0
        while i < 10:
            input_file = open(
                config.ROOT_DIR + "/Resources/Input/Top1000_Python201" +
                str(i) + "_Complete.json", 'r')
            ...
            for item in json_decode:
                if item.get('full_name').replace('/', '-') == name:
                    self.number_type_annotations_per_repo[item.get(
                        'html_url'
                    )] = self.number_type_annotations_per_repo[name]
                    del self.number_type_annotations_per_repo[name]

                    if name in self.typeLastProjectVersion_dict:
                        self.typeLastProjectVersion_dict[item.get(
                            'html_url'
                        )] = self.typeLastProjectVersion_dict[name]
        ...
        for stat in process_statistics:
            try:
                ...
                self.commit_year_dict = dict(
                    merge_dictionaries([
                        dict(self.commit_year_dict),
                        dict(stat.commit_year_dict)
                    ]))
                self.loc_year_edit = dict(
                    merge_dictionaries(
                        [dict(self.loc_year_edit),
                         dict(stat.loc_year_edit)]))

                # RQ1
                self.insert_types = dict(
                    merge_dictionaries(
                        [dict(self.insert_types),
                         dict(stat.insert_types)]))
                self.remove_types = dict(
                    merge_dictionaries(
                        [dict(self.remove_types),
                         dict(stat.remove_types)]))
                self.modify_existing_types = dict(
                    merge_dictionaries([
                        dict(self.modify_existing_types),
                        dict(stat.modify_existing_types)
                    ]))
                ...
                # RQ2.2
                self.typeAdded_dict = merge_dictionaries(
                    [self.typeAdded_dict, stat.typeAdded_dict])
                ...
                # RQ2.4
                self.typeRemoved_dict = merge_dictionaries(
                    [self.typeRemoved_dict, stat.typeRemoved_dict])

                # RQ2.5
                self.total_changed += stat.total_changed
                self.typeChanged_dict_arg = merge_dictionaries(
                    [self.typeChanged_dict_arg, stat.typeChanged_dict_arg])
                self.typeChanged_dict_var = merge_dictionaries(
                    [self.typeChanged_dict_var, stat.typeChanged_dict_var])
                self.typeChanged_dict_ret = merge_dictionaries(
                    [self.typeChanged_dict_ret, stat.typeChanged_dict_ret])
                ...
                self.number_type_annotations_per_repo = merge_dictionaries([
                    self.number_type_annotations_per_repo,
                    stat.number_type_annotations_per_repo
                ])
                ...
                self.typeLastProjectVersion_dict = merge_dictionaries([
                    self.typeLastProjectVersion_dict,
                    stat.typeLastProjectVersion_dict
                ])

                # RQ 8
                self.typeAnnotation_year_analysis = merge_dictionaries([
                    self.typeAnnotation_year_analysis,
                    stat.typeAnnotation_year_analysis
                ])
                ...
                while year < 2021:
                    self.annotation_coverage[str(
                        year)][0] += stat.annotation_coverage[str(year)][0]
                    self.annotation_coverage[str(
                        year)][1] += stat.annotation_coverage[str(year)][1]
                    self.annotation_coverage[str(
                        year)][2] += stat.annotation_coverage[str(year)][2]
                    self.annotation_coverage[str(
                        year)][3] += stat.annotation_coverage[str(year)][3]
                    self.annotation_coverage[str(
                        year)][4] += stat.annotation_coverage[str(year)][4]
                    self.annotation_coverage[str(
                        year)][5] += stat.annotation_coverage[str(year)][5]
                    year += 1
