...
import config
from papercode.TypeAnnotations.codeChange import CommitStatistics
from papercode.TypeAnnotations.codeChangeExtraction import TypeAnnotationExtractionFirstCommit, \
...
from papercode.TypeAnnotations.Utils import write_in_json, convert_list_in_list_of_dicts
from papercode.TypeErrors.TypeAnnotationCounter import count_type_annotations, extract_from_file


def repo_cloning(filenameInput: str, pathOutput: str,
                 count: List[int]) -> None:
...
def repo_cloning_oneplus(filenameInput: str, pathOutput: str,
                         count: List[int]) -> None:
    with open(filenameInput) as fh:
        articles = json.load(fh)

    article_urls = [
        article['typeLastProjectVersion_dict'] for article in articles
    ]
...
def repo_cloning_csv(pathOutput: str) -> None:
    ...
    with open(config.ROOT_DIR +
              '/Resources/Input/topJavaMavenProjects.csv') as f:
...
def query_repo_get_changes(repo_name):  # statistics, pointer, dirlist_len):
    ...
    try:
        ...
        if statistics.typeLastProjectVersion_total > 0:
            ...
            commit_temp = 'e0'
            for commit in repo.walk(last_commit,
                                    GIT_SORT_TOPOLOGICAL | GIT_SORT_REVERSE):
                try:

                    commit_year = time.strftime(
                        '%Y-%m-%d %H:%M:%S',
                        time.localtime(commit.commit_time))[:4]
                    commit_month = time.strftime(
                        '%Y-%m-%d %H:%M:%S',
                        time.localtime(commit.commit_time))[5:7]
                    commit_day = time.strftime(
                        '%Y-%m-%d %H:%M:%S',
                        time.localtime(commit.commit_time))[8:10]
                    ...
                    try:
                        if commit.author.email not in statistics.dev_dict_total:
                            statistics.dev_dict_total[str(
                                commit.author.email)] = 1
                        else:
                            statistics.dev_dict_total[str(
                                commit.author.email)] += 1
                    except:
                    ...
                    num_parents = len(
                        commit.parents
                    )  # Do not want to include merges for now, hence we check if the number of parents is 'one'
                    if num_parents >= 0:  # and commit_message_contains_query(commit.message, query_terms):
                        ...
                        if num_parents == 1:
                            ...
                            try:
                                if commit_year not in statistics.loc_year_edit:
                                    statistics.loc_year_edit[
                                        commit_year] = diff.stats.deletions + diff.stats.insertions
                                else:
                                    statistics.loc_year_edit[
                                        commit_year] += diff.stats.deletions + diff.stats.insertions
                            except:
                            ...
                            try:
                                if commit_year not in statistics.loc_year_edit:
                                    statistics.loc_year_edit[
                                        commit_year] = diff.stats.insertions
                                else:
                                    statistics.loc_year_edit[
                                        commit_year] += diff.stats.insertions
                            except:
                        ...
                        for patch in diff:
                            ...
                            TypeAnnotationExtractionLast_life(
                                config.ROOT_DIR + "/GitHub/",
                                repo_name,
                                commit,
                                patch,
                                remote_url + '/commit/' + commit.hex +
                                '#diff-' + diff.patchid.hex,
                                statistics,  # lock, logging,
                                at_least_one_type_change,
                                statistics.code_changes,
                    ...
                    # RQ 4.1
                    try:
                        if typeannotation_line_inserted[
                                0] - typeannotation_line_changed[0] > 0:
                            added_per_commit_percentage = (
                                typeannotation_line_inserted[0] -
                                typeannotation_line_changed[0]) / (
                                    tot_line_inserted -
                                    typeannotation_line_changed[0]) * 100

                            if added_per_commit_percentage <= 100:
                                statistics.list_typeAnnotation_changed_per_commit.append(
                                    added_per_commit_percentage)
                            else:
                    ...
                    # RQ 4.2
                    try:
                        if typeannotation_line_removed[
                                0] - typeannotation_line_changed[0] > 0:

                            removed_per_commit_percentage = (
                                typeannotation_line_removed[0] -
                                typeannotation_line_changed[0]) / (
                                    tot_line_removed -
                                    typeannotation_line_changed[0]) * 100
                    ...
                    try:
                        if typeannotation_line_changed[0] > 0:
                            changed_per_commit_percentage = (
                                typeannotation_line_changed[0]) / (
                                    tot_line_removed + tot_line_inserted -
                                    typeannotation_line_changed[0]) * 100
                    ...
                    if len(statistics.code_changes) > old_len:
                        ...
                        if added_per_commit_percentage <= 100 and removed_per_commit_percentage <= 100 and changed_per_commit_percentage <= 100:
                            try:
                                temp = CommitStatistics(
                                    str(remote_url + '/commit/' + commit.hex),
                                    str(commit_year),
                                    str(
                                        len(statistics.code_changes) -
                                        old_len),
                                    str(round(added_per_commit_percentage, 1))
                                    + ' %',
                                    str(round(removed_per_commit_percentage,
                                              1)) + ' %',
                                    str(round(changed_per_commit_percentage,
                                              1)) + ' %',
                        ...
                        if commit_year not in statistics.typeAnnotation_commit_annotation_year_analysis:
                            statistics.typeAnnotation_commit_annotation_year_analysis[
                                commit_year] = 1
                        else:
                            statistics.typeAnnotation_commit_annotation_year_analysis[
                                commit_year] += 1
                        ...
                        if commit_year not in statistics.typeAnnotation_commit_not_annotation_year_analysis:
                            statistics.typeAnnotation_commit_not_annotation_year_analysis[
                                commit_year] = 0

                        # RQ8
                        if commit_year not in statistics.typeAnnotation_year_analysis:
                            statistics.typeAnnotation_year_analysis[
                                commit_year] = len(
            ...
            for commit in repo.walk(last_commit,
                                    GIT_SORT_TOPOLOGICAL | GIT_SORT_REVERSE):
                commit_year = time.strftime('%Y-%m-%d %H:%M:%S',
                                            time.localtime(
                                                commit.commit_time))[:4]
        ...
        try:
            if len(statistics.dict_funct_call_no_types) > 0:
                ...
                fuct_no_type_avg = np.array(
                    list(statistics.dict_funct_call_no_types.values())).mean()
            else:
                fuct_no_type_avg = 0

            if len(statistics.dict_funct_call_types) > 0:
                fuct_type_avg = np.array(
                    list(statistics.dict_funct_call_types.values())).mean()
            else:
                fuct_type_avg = 0
        except Exception as e:
            fuct_no_type_avg = -1
        ...
        statistics.addRepo(
            repo_name, tot_this_repo_commit,
            statistics.number_type_annotations_per_repo[repo_name],
            n_test_files, n_non_test_files, len(statistics.dev_dict_total),
            fuct_no_type_avg, fuct_type_avg)
        if at_least_one_type_change[0] > 0:
            statistics.repo_with_types_changes += 1

        try:
            if statistics.total_typeAnnotation_codeChanges > 0:
                statistics.typeLastProjectVersion_percentage.append(
                    round(
                        statistics.typeLastProjectVersion_total /
                        sum(statistics.insert_types.values()) * 100, 2))

                git_checkout(config.ROOT_DIR + "/GitHub/" + repo_name,
                             commit_set, statistics)
        ...
        statistics_final.merge_results([statistics], code_changes,
                                       commit_statistics)
        ...
        write_in_json(
            config.ROOT_DIR +
            f"/Resources/Output/typeAnnotationChanges_{repo_name}.json",
            convert_list_in_list_of_dicts(code_changes))

        write_in_json(
            config.ROOT_DIR +
            f"/Resources/Output/typeAnnotationCommitStatistics_{repo_name}.json",
        ...
        if config.NORMAL_PRINT:
            print(
                "[Finished]", repo_name, "with",
                commit_with_annotations_this_repo, '/', tot_this_repo_commit,
...
def git_checkout(repo_dir, commit_set, statistics):
    ...
    for commit in commit_set:
        try:
            subprocess.run(f"git checkout {commit[4:]} --quiet".split(" "),
                           cwd=repo_dir)
            ...
            statistics.annotation_coverage[str(
                commit[:4])][1] += non_param_types
            statistics.annotation_coverage[str(commit[:4])][2] += return_types
            statistics.annotation_coverage[str(
                commit[:4])][3] += non_return_types
            statistics.annotation_coverage[str(
                commit[:4])][4] += variable_types
            statistics.annotation_coverage[str(
                commit[:4])][5] += non_variable_types
            ...
            if str(filepath).endswith(".py"):
                ...
                try:
                    ...
                    for key in non_return_types:
                        statistics.dict_funct_call_no_types[
                            key] = function_call_count(repo_dir, str(key[-1]))

                    for key in return_types:
                        statistics.dict_funct_call_types[
                            key] = function_call_count(repo_dir, str(key[-1]))
                    ...
                    for func in func_list:
                        ...
                        tot_types = len(param_types) + len(return_types) + len(
                            variable_types)
                        tot_non_types = len(non_return_types) + len(
                            non_variable_types) + len(non_param_types)
                        ...
                        statistics.matrix_files_annotations = np.append(
                            statistics.matrix_files_annotations,
                            np.array([[fuct_count, coverage * 100,
                                       tot_types]]),
                            axis=0)
        ...
        if func:
            if not check:
                if sum(brackets.values()
                       ) == 0 and not string and not multiline:
...
def error_check():
    ...
    for folder in my_list:
        ...
        result = subprocess.run(
            ['flake8', config.ROOT_DIR + '/GitHub/' + folder, '--count'],
            stdout=subprocess.PIPE)

        last = io.StringIO(result.stdout.decode('utf-8')).readlines()[-1]

        result2 = subprocess.run([
            'git', '--git-dir=' + config.ROOT_DIR + '/GitHub/' + folder +
    ...
    write_in_json(
        config.ROOT_DIR + "/Resources/Output/error_check_flake8.json",
        dict_new)
...
def stub_an():
    ...
    plt.savefig(config.ROOT_DIR + '/Resources/stub/stub_fig.pdf',
                bbox_inches='tight')
    ...
    for i in directory_list:
        ...
        try:
            ...
            for filepath in pathlib.Path(config.ROOT_DIR + "/GitHub4/" +
                                         i).glob('**/*'):
                try:
                    if str(filepath).endswith(".pyi"):
                        ...
                        with open(
                                config.ROOT_DIR + '/Resources/stub/' + str(i),
                                'w') as filehandle:
                            filehandle.write('%s\n' % var_list)
                            filehandle.write('%s\n' % arg_list)
                            filehandle.write('%s\n' % ret_list)
    ...
    with open(config.ROOT_DIR + '/Resources/stub/000final.txt',
              'w') as filehandle:
        # filehandle.write('%s\n' % arg_num)
        # filehandle.write('%s\n' % var_num)
        # filehandle.write('%s\n' % ret_num)
