...
import statistics
import config
from papercode.TypeAnnotations.codeStatistics import CodeStatistics
from papercode.TypeAnnotations.Utils import *
import numpy as np
import pandas as pd
import json
...
def write_results(statistics, code_changes, commit_statistics):
    write_in_json(
        config.ROOT_DIR + "/Resources/Output/typeAnnotationChanges.json",
        convert_list_in_list_of_dicts(code_changes))

    print('\nCode changes with type annotations found: ' +
          str(len(code_changes)))

    write_in_json(
        config.ROOT_DIR +
        "/Resources/Output/typeAnnotationCommitStatistics.json",
    ...
    write_in_json(
        config.ROOT_DIR +
        "/Resources/Output/typeAnnotationAllStatisticsRAW.json",
...
def compute_correlations(commits_stars_annotations):
    ...
    projects.columns = [
        "commits", "stars", "annotations", "n_forks", "n_issues",
        "n_test_files", "n_non_test_files", "n_dev", "funct_type_avg",
        "fuct_no_type_avg"
    ]
    projects_with_annotations = projects[projects.annotations > 0]
    print(
        f"Computing correlations across {len(projects)} projects, {len(projects_with_annotations)} with annotations "
    )
    print(f"  All projects:")
    print(
        f"    Correlation between annotations and commits: {projects['commits'].corr(projects['annotations'])}"
    )
    print(
        f"    Correlation between annotations and stars: {projects['commits'].corr(projects['stars'])}"
    )
...
def compute_correlations2(commits_stars_annotations):
    ...
    print(f"  All projects:")
    print(
        f" no test   Correlation between annotations and funct: {projects['funct'].corr(projects['annotations'])}"
    )
    print(
        f" no test    Correlation between cov and funct: {projects['funct'].corr(projects['cov'])}"
    )
    print(f"  Projects with annotations:")
    print(
...
def myplot(statistics):
    ...
    statistics.typeChanged_dict_var = sum_type_changes(
        statistics.typeChanged_dict_var)
    statistics.typeChanged_dict_arg = sum_type_changes(
        statistics.typeChanged_dict_arg)
    statistics.typeChanged_dict_ret = sum_type_changes(
    ...
    bar_plot_xy(config.ROOT_DIR + "/Resources/Output/TopChanged_arg.pdf",
                ...
                list(statistics.typeChanged_dict_arg.values())[:5],
                'Type changes in function arguments',
                'Occurrences\n(log scale)',
                ylim=50000)
    ...
    bar_plot_xy(config.ROOT_DIR + "/Resources/Output/TopChanged_ret.pdf",
                ...
                list(statistics.typeChanged_dict_ret.values())[:5],
                'Type changes in function return',
                'Occurrences\n(log scale)',
                ylim=50000)
    ...
    bar_plot_xy(config.ROOT_DIR + "/Resources/Output/TopChanged_var.pdf",
                ...
                list(statistics.typeChanged_dict_var.values())[:5],
                'Type changes in variable assignment',
                'Occurrences\n(log scale)',
                ylim=50000)

    smooth_line_xy_multi(
        config.ROOT_DIR + "/Resources/Output/elements_annotated.pdf",
        statistics.annotation_coverage,
        x_label="Year",
        y_label="Type annotation coverage",
        title=
        "Presence of type annotations in\nthe last version of the repositories.",
        color1='blue',
        color2='red',
        xlim=None,
        ylim=None)
    ...
    for lll in statistics.annotation_coverage:
        ...
        llll.append(statistics.annotation_coverage[lll][0] +
                    statistics.annotation_coverage[lll][2] +
                    statistics.annotation_coverage[lll][4])
        loc.append((statistics.annotation_coverage[lll][0] +
                    statistics.annotation_coverage[lll][2] +
                    statistics.annotation_coverage[lll][4]) /
                   ((statistics.annotation_coverage[lll][1] +
                     statistics.annotation_coverage[lll][3] +
                     statistics.annotation_coverage[lll][5] + 1) / 1000))

    smooth_line_xy_double(config.ROOT_DIR +
                          "/Resources/Output/annotationsPerYear2.pdf",
                          years,
                          llll,
                          loc,
                          x_label="Year",
    ...
    statistics.commit_year_dict = dict(
        sort_dictionary(statistics.commit_year_dict))
    ...
    for dictionary in statistics.list_dev_plot:
        ...
        for key in dictionary:
            if dictionary[key] == 0:
                list_link_repo.append("https://api.github.com/repos/" +
                                      key.replace("-", "/", 1))
        ...
        list_top_1_developers.append(
            float(dictionary[key] / statistics.list_dev_dict_total[i][key] *
                  100))
    ...
    histogram_plot_xy2(
        config.ROOT_DIR +
        "/Resources/Output/perc_annotations_lines_per_commit.pdf",
...
def load_final_statistics():
    file = open(config.ROOT_DIR +
                "/Resources/Output/typeAnnotationChanges.json")
    fh = file.read()
    ...
    try:
        ...
        histogram_plot_xy(config.ROOT_DIR +
                          "/Resources/Output/num_changes.pdf",
                          n_changes,
                          'Number of changes for a type annotation',
                          'Type annotations (log scale)',
                          'linear',
                          'log',
                          bins=26)
    except:
    ...
    with open(config.ROOT_DIR +
              "/Resources/Output/typeAnnotationAllStatisticsRAW.json") as fh:
    ...
    finalStatistics.repo_with_types_changes = allStatistics[0][
        'repo_with_types_changes']
    finalStatistics.commits_with_typeChanges = allStatistics[0][
        'commits_with_typeChanges']
    finalStatistics.total_typeAnnotation_codeChanges = allStatistics[0][
        'total_typeAnnotation_codeChanges']

    # RQ1
    finalStatistics.insert_types = allStatistics[0]['insert_types']
    finalStatistics.remove_types = allStatistics[0]['remove_types']
    finalStatistics.modify_existing_types = allStatistics[0][
    ...
    finalStatistics.binarySequenceType_added = allStatistics[0][
        'binarySequenceType_added']
    finalStatistics.textSequenceType_added = allStatistics[0][
        'textSequenceType_added']
    ...
    # RQ2.2
    finalStatistics.typeAdded_dict = dict(
        tuple(tuple(x) for x in allStatistics[0]['typeAdded_dict']))
    ...
    finalStatistics.numericType_removed = allStatistics[0][
        'numericType_removed']
    finalStatistics.binarySequenceType_removed = allStatistics[0][
        'binarySequenceType_removed']
    finalStatistics.textSequenceType_removed = allStatistics[0][
        'textSequenceType_removed']
    finalStatistics.mappingTypes_removed = allStatistics[0][
        'mappingTypes_removed']
    finalStatistics.setTypes_removed = allStatistics[0]['setTypes_removed']
    finalStatistics.sequenceType_removed = allStatistics[0][
        'sequenceType_removed']
    ...
    # RQ2.4
    finalStatistics.typeRemoved_dict = dict(
        tuple(tuple(x) for x in allStatistics[0]['typeRemoved_dict']))

    # RQ2.5
    finalStatistics.total_changed = allStatistics[0]['total_changed']
    finalStatistics.typeChanged_dict_ret = dict(
        tuple(tuple(x) for x in allStatistics[0]['typeChanged_dict_ret']))
    finalStatistics.typeChanged_dict_var = dict(
        tuple(tuple(x) for x in allStatistics[0]['typeChanged_dict_var']))
    finalStatistics.typeChanged_dict_arg = dict(
        tuple(tuple(x) for x in allStatistics[0]['typeChanged_dict_arg']))

    # RQ 3.1
    finalStatistics.functionArgsType_added = allStatistics[0][
        'functionArgsType_added']
    finalStatistics.functionReturnsType_added = allStatistics[0][
        'functionReturnsType_added']
    finalStatistics.variableType_added = allStatistics[0]['variableType_added']

    # RQ 3.2
    finalStatistics.functionArgsType_removed = allStatistics[0][
        'functionArgsType_removed']
    finalStatistics.functionReturnsType_removed = allStatistics[0][
        'functionReturnsType_removed']
    finalStatistics.variableType_removed = allStatistics[0][
        'variableType_removed']

    # RQ 3.3
    finalStatistics.functionArgsType_changed = allStatistics[0][
        'functionArgsType_changed']
    finalStatistics.functionReturnsType_changed = allStatistics[0][
        'functionReturnsType_changed']
    finalStatistics.variableType_changed = allStatistics[0][
        'variableType_changed']

    # RQ 4.1
    finalStatistics.typeAnnotation_added_per_commit = allStatistics[0][
        'typeAnnotation_added_per_commit']
    finalStatistics.list_typeAnnotation_added_per_commit = allStatistics[0][
        'list_typeAnnotation_added_per_commit']

    # RQ 4.2
    finalStatistics.typeAnnotation_removed_per_commit = allStatistics[0][
        'typeAnnotation_removed_per_commit']
    finalStatistics.list_typeAnnotation_removed_per_commit = allStatistics[0][
        'list_typeAnnotation_removed_per_commit']

    # RQ 4.3
    finalStatistics.typeAnnotation_changed_per_commit = allStatistics[0][
        'typeAnnotation_changed_per_commit']
    finalStatistics.list_typeAnnotation_changed_per_commit = allStatistics[0][
        'list_typeAnnotation_changed_per_commit']

    # RQ 4.4
    finalStatistics.annotation_related_insertion_edits_vs_all_commit = allStatistics[
        0]['annotation_related_insertion_edits_vs_all_commit']

    # RQ 4.5
    finalStatistics.annotation_related_deletion_edits_vs_all_commit = allStatistics[
        0]['annotation_related_deletion_edits_vs_all_commit']

    # RQ 5
    finalStatistics.matrix_commits_stars_annotations = np.array(
        allStatistics[0]['matrix_commits_stars_annotations'])
    finalStatistics.matrix_files_annotations = np.array(
        allStatistics[0]['matrix_files_annotations'])
    finalStatistics.matrix_test_files_annotations = np.array(
        allStatistics[0]['matrix_test_files_annotations'])

    # RQ 6
    finalStatistics.number_type_annotations_per_repo = dict(
        tuple(
            tuple(x)
            for x in allStatistics[0]['number_type_annotations_per_repo']))

    # RQ 7
    finalStatistics.typeLastProjectVersion_total = allStatistics[0][
        'typeLastProjectVersion_total']
    finalStatistics.typeLastProjectVersion_percentage = allStatistics[0][
        'typeLastProjectVersion_percentage']
    finalStatistics.typeLastProjectVersion_dict = dict(
        tuple(
            tuple(x) for x in allStatistics[0]['typeLastProjectVersion_dict']))

    # RQ 8
    finalStatistics.typeAnnotation_year_analysis = allStatistics[0][
        'typeAnnotation_year_analysis']

    # RQ 9
    finalStatistics.typeAnnotation_commit_annotation_year_analysis = allStatistics[
        0]['typeAnnotation_commit_annotation_year_analysis']

    finalStatistics.typeAnnotation_commit_not_annotation_year_analysis = allStatistics[
        0]['typeAnnotation_commit_not_annotation_year_analysis']

    # RQ 10
    finalStatistics.annotation_coverage = allStatistics[0][
        'annotation_coverage']

    # RQ 11
    finalStatistics.list_dev_dict = allStatistics[0]['list_dev_dict']
    finalStatistics.list_dev_dict_total = allStatistics[0][
        'list_dev_dict_total']
